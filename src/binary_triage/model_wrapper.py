...
que o artefato salvo seja autossuficiente: `predict()` já aplica o threshold
calibrado em vez do padrão 0.5 do scikit-learn, sem quem consome o modelo
precisar saber qual threshold foi escolhido nem recriar lógica externa.

Opcionalmente carrega também `specialty_pipeline`: um classificador
multiclasse (oncologia/cardiologia/neurologia/gastroenterologia) treinado só
nas linhas ESPECIALISTA. A decisão binária não depende dele — ele só
responde "para qual especialidade, dado que não é clínica geral", via
`predict_specialty_proba`/`predict_detailed`. Sem isso, o modelo diria
"precisa de especialista" sem dizer qual, pouco acionável numa triagem real.
"""
...
class ThresholdedBinaryClassifier(BaseEstimator, ClassifierMixin):
    def __init__(
        ...
        pipeline,
        threshold: float = 0.5,
        positive_label: str = "ESPECIALISTA",
        negative_label: str = "CLINICO_GERAL",
        specialty_pipeline=None,
    ):
    ...
    def predict_proba(self, x):
        ...
        neg_idx, pos_idx = classes.index(self.negative_label), classes.index(self.positive_label)
    ...
    def predict_specialty_proba(self, x):
        """Probabilidade de cada especialidade, independente da decisão binária.

        Só faz sentido interpretar quando `predict(x)` == ESPECIALISTA para a
        mesma linha.
        """

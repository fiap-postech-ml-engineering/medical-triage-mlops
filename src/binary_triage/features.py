...
As listas de termos abaixo vêm da EDA (`src/binary_triage/eda.py`, seção 6 —
top termos por especialidade) e não são um vocabulário inventado: são os
termos que mais diferenciam cada especialidade neste corpus específico.
"""
...
def extract_meta_features(texts: pd.Series) -> pd.DataFrame:
    ...
    for text in texts:
        ...
        kw_onco = _count_keywords(text_lower, ONCOLOGY_TERMS)
        kw_cardio = _count_keywords(text_lower, CARDIO_TERMS)
        kw_neuro = _count_keywords(text_lower, NEURO_TERMS)
        kw_gastro = _count_keywords(text_lower, GASTRO_TERMS)
...
class TextMetaFeatures(BaseEstimator, TransformerMixin):
    ...
    Recebe a coluna de texto (Series/array 1D) e devolve a matriz de
    features estruturadas derivadas do texto.
    """

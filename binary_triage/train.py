...
Uso: `uv run python -m binary_triage.train` (a partir da raiz do projeto).
"""
...
from binary_triage.features import TextMetaFeatures  # noqa: E402
from binary_triage.model_wrapper import ThresholdedBinaryClassifier  # noqa: E402

logging.basicConfig(level=logging.INFO, format="%(asctime)s %(levelname)s %(message)s")
logger = logging.getLogger(__name__)

BASE_DIR = Path(__file__).resolve().parent.parent
RAW_PATH = BASE_DIR / "data" / "raw" / "laudos.csv"
    ...
    return Pipeline(
        [
            (
                "features",
                ColumnTransformer(
                    [("tfidf", TfidfVectorizer(max_features=max_features, ngram_range=(1, 2), min_df=2), "texto")]
                ),
            ),
            ("clf", clf),
        ]
        ...
        [
            (
                ...
                ColumnTransformer(
                    [
                        ("tfidf", TfidfVectorizer(max_features=max_features, ngram_range=(1, 2), min_df=2), "texto"),
                        ("meta", Pipeline([("extract", TextMetaFeatures()), ("scale", StandardScaler())]), "texto"),
                    ]
                ),
            ),
            ("clf", clf),
...
def make_lsa(clf, n_components=200):
    return Pipeline(
        [
            (
                "features",
                ColumnTransformer(
                    [("tfidf", TfidfVectorizer(max_features=30000, ngram_range=(1, 2), min_df=2), "texto")]
                ),
            ),
...
def evaluate(pipeline, X, y_true, threshold=0.5):
    ...
    return {
        "precision_especialista": precision_score(y_true, y_pred, pos_label=POSITIVE, zero_division=0),
...
def main():
    ...
    # ---- 1) Comparação de candidatos diretos no conjunto de validação ----
    logger.info("=" * 80)
    ...
    # ---- 2) Arquitetura em duas etapas: multiclasse -> binário ----
    ...
    logger.info("  two_stage_multiclass -> %s", {k: round(v, 4) for k, v in metrics_2stage.items()})
    ...
    # ---- 3) Escolha do melhor candidato direto (exclui two-stage da escolha
    #         principal pois será comparado separadamente como arquitetura) ----
    ...
    logger.info(
        ">>> Comparação direto (%s) vs two-stage: recall_especialista direto=%.4f | two-stage=%.4f | "
        "f1_macro direto=%.4f | two-stage=%.4f",
        best_name,
        direct_df.loc[best_name, "recall_especialista"],
        comparison_df.loc["two_stage_multiclass", "recall_especialista"],
        direct_df.loc[best_name, "f1_macro"],
        comparison_df.loc["two_stage_multiclass", "f1_macro"],
    )

    # ---- 4) Tuning de threshold no modelo escolhido, usando validação ----
    logger.info("=" * 80)
    ...
    # Regra: entre os thresholds com recall_especialista >= 0.90, escolhe o
    # que maximiza f1_macro; se nenhum atingir 0.90, usa o de maior recall.
    safe = threshold_df[threshold_df["recall_especialista"] >= 0.90]
    if len(safe) > 0:
        final_threshold = safe["f1_macro"].idxmax()
    else:
        final_threshold = threshold_df["recall_especialista"].idxmax()
    logger.info(">>> Threshold final escolhido: %.2f", final_threshold)
    logger.info(">>> Métricas em validação nesse threshold: %s", threshold_df.loc[final_threshold].round(4).to_dict())

    # ---- 5) Refit no treino+validação, avaliação final única no teste ----
    logger.info("=" * 80)
    logger.info("REFIT EM TREINO+VALIDAÇÃO E AVALIAÇÃO FINAL NO TESTE (nunca visto até aqui)")
    logger.info("=" * 80)
    ...
    logger.info("Métricas finais no TESTE (threshold=%.2f): %s", final_threshold, {k: round(v, 4) for k, v in test_metrics.items()})
    ...
    # Também reporta o teste com threshold=0.5 para deixar explícito o efeito do tuning
    test_metrics_05, _, _ = evaluate(final_pipeline, X_test, y_test, threshold=0.5)
    logger.info("Para comparação — métricas no TESTE com threshold padrão 0.5: %s", {k: round(v, 4) for k, v in test_metrics_05.items()})
    ...
    plt.close(fig)
    logger.info("Matriz de confusão (teste):\n%s", pd.DataFrame(cm, index=[f"real_{NEGATIVE}", f"real_{POSITIVE}"], columns=[f"pred_{NEGATIVE}", f"pred_{POSITIVE}"]))

    # PR curve
    precisions, recalls, pr_thresholds = precision_recall_curve((y_test == POSITIVE).astype(int), p_pos_test)
    fig, ax = plt.subplots(figsize=(5, 5))
    ...
    # ---- 7) Interpretabilidade: top termos/coeficientes ----
    logger.info("=" * 80)
    ...
    logger.info("\n%s", "\n".join(interpret_lines[:60]))

    # ---- 8) Exemplos de previsão + análise de erros no teste ----
    examples_md = build_examples_and_error_analysis(final_pipeline, test_df, y_pred_test, p_pos_test, final_threshold)
    with open(REPORTS_DIR / "examples_and_errors.md", "w", encoding="utf-8") as f:
        f.write(examples_md)
    logger.info("Exemplos e análise de erros salvos em %s", REPORTS_DIR / "examples_and_errors.md")

    # ---- 9) Sub-classificação por especialidade (dado ESPECIALISTA, qual delas?) ----
    # A decisão binária (seções 1-8) só diz "precisa de especialista" — não diz qual.
    # Isso é pouco acionável numa triagem real, então treinamos um segundo classificador,
    # só nas linhas ESPECIALISTA, para responder "qual especialidade" (oncologia,
    # cardiologia, neurologia, gastroenterologia). Decisão binária e sub-classificação
    # são independentes: mesmo split, mas o sub-modelo nunca vê linhas CLINICO_GERAL.
    logger.info("=" * 80)
    ...
    logger.info("Distribuição de especialidade (treino, apenas ESPECIALISTA): %s", esp_train["especialidade"].value_counts().to_dict())
    ...
    logger.info("Relatório de classificação por especialidade (teste, só linhas ESPECIALISTA):\n%s", specialty_report)
    ...
    plt.close(fig)
    logger.info("Matriz de confusão por especialidade salva em %s", REPORTS_DIR / "confusion_matrix_specialty_test.png")

    # ---- 10) Salvar modelo final (decisão binária + sub-classificação por especialidade) ----
    ...
    logger.info(">>> Modelo final salvo em %s (com sub-classificador de especialidade embutido)", MODEL_PATH)
...
def interpret_model(pipeline: Pipeline, model_name: str):
    ...
    if hasattr(clf, "calibrated_classifiers_"):
        lines.append("Modelo calibrado (LinearSVC + sigmoid) — coeficientes extraídos do estimador base.")
        base = clf.calibrated_classifiers_[0].estimator
        coefs = base.coef_[0]
    elif hasattr(clf, "coef_"):
        coefs = clf.coef_[0]
    else:
        lines.append("Modelo não-linear ou pós-SVD: coeficientes não são diretamente interpretáveis por termo.")
        return lines
...
def build_examples_and_error_analysis(pipeline, test_df, y_pred, p_pos, threshold):
    ...
    for _, row in df[df["acerto"]].sample(min(3, df["acerto"].sum()), random_state=RANDOM_SEED).iterrows():
        lines.append(
    ...
    lines.append(f"\n## Falsos negativos (real=ESPECIALISTA, previsto=CLINICO_GERAL) — {len(fn)} de {len(df)} no teste\n")
    lines.append(
        "Impacto clínico: paciente que precisava de especialista é encaminhado ao clínico geral — "
        "risco de atraso diagnóstico/terapêutico. É o erro mais custoso neste contexto.\n"
    )
    for _, row in fn.sample(min(5, len(fn)), random_state=RANDOM_SEED).iterrows():
        lines.append(
            f"- especialidade original: {row['especialidade']} | p(ESPECIALISTA)={row['p_especialista']:.3f}\n"
            f"  > {row['texto'][:220]}...\n"
        )

    lines.append(f"\n## Falsos positivos (real=CLINICO_GERAL, previsto=ESPECIALISTA) — {len(fp)} de {len(df)} no teste\n")
    lines.append(
    ...
    for _, row in fp.sample(min(5, len(fp)), random_state=RANDOM_SEED).iterrows():
        lines.append(f"- p(ESPECIALISTA)={row['p_especialista']:.3f}\n  > {row['texto'][:220]}...\n")
